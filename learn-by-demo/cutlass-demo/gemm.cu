...
void gemm(char transA, char transB, int m, int n, int k, Alpha alpha,
          TA const* A, TB const* B, Beta beta, TC* C, cudaStream_t stream = 0) {
  if (transA == 'N' && transB == 'T') {
    return gemm_nt(m, n, k, alpha, A, m, B, n, beta, C, m, stream);
  } else if (transA == 'T' && transB == 'N') {
    return gemm_tn(m, n, k, alpha, A, k, B, k, beta, C, m, stream);
  }
...
int main(int argc, char** argv) {
  ...
  // Run once
  d_C = h_C;
  gemm(transA, transB, m, n, k, alpha, d_A.data(), d_B.data(), beta,
       d_C.data());
  CUTE_CHECK_LAST();
  std::vector<TC> cute_result(d_C.size());
  d_C.To(cute_result);

...
class FuseQatConvBNReLU:
    ...
    def apply(graph_module: torch.fx.GraphModule):
        for maybe_relu_node in graph_module.graph.nodes:
            if FuseQatConvBNReLU.get_module(graph_module, maybe_relu_node, nn.ReLU) is None:
                continue
            ...
            maybe_bn_node = maybe_fq_node.args[0]
            ...
            maybe_fq1_node = maybe_bn_node.args[0]
            if FuseQatConvBNReLU.get_module(graph_module, maybe_fq1_node, (ObserverBase, FakeQuantizeBase)) is None:
                continue
            if len(maybe_fq1_node.users) > 1:
                continue

            maybe_conv_node = maybe_fq1_node.args[0]
            ...
            maybe_fq1_node.replace_all_uses_with(maybe_conv_node)
            graph_module.graph.erase_node(maybe_fq_node)
            graph_module.graph.erase_node(maybe_fq1_node)
